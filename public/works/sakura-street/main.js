...
function createSakuraTree(x, z, scale = 1) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        const crownGeo = new THREE.SphereGeometry(size, 8, 8);
...
function createGroundPetalCluster(x, z, count) {
    for (let i = 0; i < count; i++) {
        ...
        const petalGeo = new THREE.PlaneGeometry(size, size);
        const petalMat = new THREE.MeshBasicMaterial({
            color: new THREE.Color().setHSL(0.95 + Math.random() * 0.05, 0.6, 0.85),
            side: THREE.DoubleSide,
            transparent: true,
            opacity: 0.7
        });
        const petal = new THREE.Mesh(petalGeo, petalMat);
        petal.position.set(px, 0.01, pz);
...
for (let z = -20; z < 20; z += 5) {
    createGroundPetalCluster(-2 + Math.random() * 4, z, 8);
}
...
function createGreenTree(x, z, scale) {
    ...
    const leavesGeo = new THREE.SphereGeometry(0.8 * scale, 8, 8);
...
function createLampPost(x, z) {
    ...
    const poleGeo = new THREE.CylinderGeometry(0.05, 0.06, 3, 8);
